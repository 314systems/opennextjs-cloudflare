...
class StaticAssetsIncrementalCache implements IncrementalCache {
	...
	async set<CacheType extends CacheEntryType = "cache">(
		...
		error(
			`StaticAssetsIncrementalCache: Failed to set to read-only cache key=${key} type=${String(cacheType)}`
		);
	}
...
const staticAssetsIncrementalCache: StaticAssetsIncrementalCache = new StaticAssetsIncrementalCache();
