 ...
 * @param args
 */
export async function deployCommand(args: WithWranglerArgs<{ cacheChunkSize?: number | undefined }>): Promise<void> {
	printHeaders("deploy");
 ...
 */
export function addDeployCommand<T>(y: Argv<T>): Argv<T> {
	return y.command(
		"deploy [args..]",
